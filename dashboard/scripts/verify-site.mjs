...
check("inquiry moved to dedicated page", !main.includes('id="inquire"'));
check("CTAs link the branch inquiry page", main.includes('href="/b/main/inquire"'));
...
console.log("\n=== inquiry pages (dedicated, shareable) ===");
const { html: orgInq } = await getPage("/inquire");
check("org inquiry page renders", orgInq.includes("Send on WhatsApp"));
check("org-level branch picker", orgInq.includes("Which location are you asking about?"));
check("honeypot present", orgInq.includes('id="company"'));
const { html: mainInq } = await getPage("/b/main/inquire");
check("branch inquiry page renders", mainInq.includes("Send on WhatsApp"));
check("branch inquiry pre-scoped (no picker)", !mainInq.includes("Which location are you asking about?"));
check("branch inquiry honeypot", mainInq.includes('id="company"'));
const preselect = await getPage("/b/main/inquire?room=2-in-1");
check("?room= pre-selection", preselect.html.includes("2-in-1"));
...
check("annex hero uses branch photos", /_next\/static\/media\/annex\.[a-f0-9]+\.(jpg|webp)/.test(annex) || annex.includes("/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fannex"));
